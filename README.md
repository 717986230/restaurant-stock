...
注册要填邀请码，登录不需要。邀请码只存在 Cloudflare 的 Secret 里，不在代码库、也不在任何配置文件里：

```bash
npx wrangler secret put INVITE_CODE
```

**没配置 `INVITE_CODE` 时注册直接关闭**（返回 503），而不是悄悄放开——
宁可自己也注册不了（补一条命令即可），也好过忘了配就变成谁都能注册。
登录不受影响，已有账号照常用。

换邀请码就是再跑一遍这条命令，已注册的账号不受影响。
邀请码同样走定时安全比较，猜错也计入同一 IP 连错 8 次锁 10 分钟的限流。

本地开发把邀请码写在 `.dev.vars` 里（该文件已被 git 忽略，可从 `.dev.vars.example` 复制）。
